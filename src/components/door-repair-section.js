...
export const DoorRepair = ({section}) => {
  const TextRender = section.content.filter(
    (component) => 
      component.internalName === "Component: Repair Text"
  )
  const SectionImage = getImage(section.content.find(component => component.internalName === "Component: Repaired Image").sectionImage);

  return(
    <Section SectionGrid Repair
...
export const query = graphql`
  fragment DoorRepairContent on ContentfulComponentSection{
    internalName
    contentful_id
    content{
